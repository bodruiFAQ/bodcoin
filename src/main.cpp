...
#if defined(NDEBUG)
# error "Bodcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0e982dc5fdec596236f53849edf057784856f27ad649ea7fb786da92a5a0b2b8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bodcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bodcoin Signed Message:\n";
...
{
    // Bodcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Bodcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Bodcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Bodcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bodcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bodcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x67296e03e4d7996413f2021b770727ade4d0788074ef332b6fe0955c3e25ade7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Former Congressman Joe Walsh Ends 2020 Republican Campaign Against Trump";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1581090878;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085717769;

        if (fTestNet)
        {
            block.nTime    = 1581090813;
            block.nNonce   = 390286746;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd3823106f6abbc30e3b4ea481743eaf7227246543b49188c85580f1034cc6b7a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Bodcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BodcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BodcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BodcoinMiner(CWallet *pwallet)
{
    printf("BodcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bodcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BodcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BodcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BodcoinMiner, pwallet));
}
